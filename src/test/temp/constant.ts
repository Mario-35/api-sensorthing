...
export const prepareToApiDoc = (input: IApiInput, Entity: string): IApiDoc => {
    return {
        ...
        // apiExample : input.result.request.url,
        apiError: input.apiError,
        apiParam: input.apiParam,
        apiSuccess: input.apiSuccess,
        apiParamExample: input.result && input.result.request._data ? JSON.stringify(input.result.request._data, null, 4) : undefined,
        apiSampleRequest: input.api.startsWith("{get}") && input.apiExample ? `proxy${input.apiExample.replace(")", "")}` : "",
        apiSuccessExample:
            input.result.type === "text/plain" ? input.result.text : input.result && input.result.body ? JSON.stringify(input.result.body, null, 4) : undefined
    };
};
