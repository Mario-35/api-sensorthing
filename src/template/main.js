...
require([
        ...
        api.forEach(function(entry) {
            if (entry.group === group) {
                if (oldName !== entry.name) {
                    nav.push({
                        title: entry.title.substring(entry.title.indexOf("]") + 2).trim(),
                        group: group,
                        name: entry.name,
                        type: entry.type,
    ...
    apiGroups.forEach(function(groupEntry) {
        ...
        api.forEach(function(entry) {
            if(groupEntry === entry.group) {
                ...
                const ind = fields.article.title.indexOf("]") ;
                if (ind > 0)
                    fields.article.title =  fields.article.title.substring(ind + 2).trim(),
  ...
  $(".sidenav").find("a").on("click", function (e) {
    ...
    $("#sidenav li[data-version='" + version + "']" + "[data-group='" + group + "']").each(function (){
